...
func main() {
	...
	repoName := "runc"
	// repoVersion := "1.1.4"
	filepath := fmt.Sprintf("{SourceCodeDir}/Source_Code/%s", repoName)

	var wholeFileNames = []string {}
	var wholeFunctions = [] string {}

	GetAllFile(filepath, &wholeFileNames)
	// fmt.Printf("%v\nFile number is %d\n", wholeFileNames, len(wholeFileNames))
	...
	ast.Inspect(node, func(n ast.Node) bool {
		...
		if ok {
			...
			if functionName, ok := fn.(*ast.Ident); ok{
				...
				// fmt.Printf("1*Called function name %s\n", singleFunction)
				}
			if functionName,ok := fn.(*ast.SelectorExpr); ok{
				...
				// fmt.Printf("2*Used function name %s\n", wholeFunctionName)
			}
		}
		return true
	})
